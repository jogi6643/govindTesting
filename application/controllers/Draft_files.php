...
class Draft_files extends MY_Controller {
	 ...
	 public function __construct() {
		...
		$this->load->model("Draft_files_model");
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = 'Draft Files';
		$data['path_url'] = 'draft_files';
		$session = $this->session->userdata('username');
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('109',$role_resources_ids)) {
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("e-file/draft_files", $data, TRUE);
    ...
    public function file_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("e-file/draft_files", $data);
		} else {
			redirect('');
		}
		...
		// print_r($role_id);die;
		 ...
		 $file = $this->Draft_files_model->get_files();	
		 }else{
		 $file = $this->Draft_files_model->get_files_user_draft($session['user_id']);
		 }
		...
		$supporting_file = $this->Draft_files_model->read_file_upload($r->id); 
	...
	{	
		...
		$result = $this->Draft_files_model->read_file_information($id);
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('e-file/dialog_draft_files', $data);
		} else {
			redirect('');
		}
	}
	...
	if($this->input->post('edit_type')=='edit_draft_file') {
		$session = $this->session->userdata('username');	
		...
		if($e_sign != null){
			...
			if($verification_code != null){
				...
				if($verification_code == $verify_code){
					...
					$result_file_out = $this->Files_out_model->add($data_file_out);		
					$employee = $this->Employees_model->read_employee_information($post_data['employee_id']);
					$data2 = array(
					'file_id' => $id,
					'reporting_officer' => $post_data['reporting_officer'],
