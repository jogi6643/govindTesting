...
class Employees extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['all_grades'] = $this->Grade_model->all_grade(); 
    ...
    public function employees_list()
     {
		...
		$grade = $this->Grade_model->read_file_information($r->grade); 
		if(!empty($grade)){
		$grade_name = $grade[0]->grade;
		}else{
		$grade_name = "";		
		}
	  ...
	  public function detail() {
		...
		$data = array(
			...
			'dsc_signature' => $result[0]->dsc_signature,
	...
	public function add_employee() {
		...
		} else if(strlen($this->input->post('first_name')) <= '3') {
		   $Return['error'] = "First name field should less than 3 characters ";
		} else if(strlen($this->input->post('first_name')) > '35') {
		   $Return['error'] = "First name field should more than 35 characters ";
		} else if($this->input->post('last_name')==='') {
			$Return['error'] = $this->lang->line('xin_employee_error_last_name');
		} else if(strlen($this->input->post('last_name')) <= '3') {
		   $Return['error'] = "Last name field should less than 3 characters ";
		} else if(strlen($this->input->post('last_name')) > '30') {
		   $Return['error'] = "Last name field should more than 30 characters ";
	...
	*/	
		...
		} else if (!filter_var($this->input->post('username'), FILTER_VALIDATE_EMAIL)) {
			$Return['error'] = $this->lang->line('xin_employee_error_invalid_email');
		} else if($this->username_exists($this->input->post('username')) == TRUE ) {
			$Return['error'] = "Username/Email already exist.";
	    } 
		else if (!preg_match('#@dcil.co.in#', $this->input->post('username'))){
		...
		} else if (!preg_match('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%]).{7,15}$/', $this->input->post('password'))){
			$Return['error'] = "Please enter password with 7-15 character (One Capital Character, One Small Character, One Number and One Special Character).";
		...
		$_FILES['signature']['size'] == 1000000;
		$ext = pathinfo($filename, PATHINFO_EXTENSION);

		if(in_array($ext,$allowed)){
		$tmp_name = $_FILES["signature"]["tmp_name"];
		$bill_copy = "./uploads/signature/";
		$lname 	= basename($_FILES["signature"]["name"]);
		$newfilename = "signature"."_".round(microtime(true)).'.'.$ext;
		//	$newfilename = $lname;
		move_uploaded_file($tmp_name, $bill_copy.$newfilename);

		if($ext != null && $_FILES['signature']['size'] > 0){
		$fname = $newfilename;
		}else{
		$fname = "";
		}
		...
		$filename2 = $_FILES['dsc_signature']['name'];
		$_FILES['dsc_signature']['size'] == 1000000;
		$ext2 = pathinfo($filename2, PATHINFO_EXTENSION);

		if(in_array($ext2,$allowed2)){
		$tmp_name2 = $_FILES["dsc_signature"]["tmp_name"];
		$bill_copy2 = "./uploads/signature/";
		$lname2 	= basename($_FILES["dsc_signature"]["name"]);
		$newfilename2 = "dsc_signature"."_".round(microtime(true)).'.'.$ext2;
		//	$newfilename = $lname;
		move_uploaded_file($tmp_name2, $bill_copy2.$newfilename2);

		if($ext2 != null && $_FILES['dsc_signature']['size'] > 0){
		$fname2 = $newfilename2;
		}else{
		$fname2 = "";
		}
...
/*			//get setting info 
			...
			<img src="'.$logo.'" title="'.$cinfo[0]->company_name.'"><br>'.str_replace(array("{var site_name}","{var site_url}","{var username}","{var employee_name}","{var password}"),array($cinfo[0]->company_name,site_url(),$this->input->post('username'),$full_name,$this->input->post('password')),htmlspecialchars_decode(stripslashes($template[0]->message))).'</div>';
				...
				$Return['error'] = 'User Created but mail notification failed'; }
...
*/				
		...
		//else{ $Return['error'] = 'File should be in jpg format or size should not be more than 20 KB'; }
		
		if(($_FILES['signature']['size'] != '0') || ($_FILES['signature']['size'] != null)){
			$fname = $newfilename;
		}else{
			$fname = $this->input->post('signature_part');
		}
		
		$allowed2 =  array('jpg','jpeg','png','gif');
		$filename2 = $_FILES['dsc_signature']['name'];
		$_FILES['dsc_signature']['size'] == 20000;
		$ext2 = pathinfo($filename2, PATHINFO_EXTENSION);
		...
		if(($_FILES['dsc_signature']['size'] != '0') || ($_FILES['dsc_signature']['size'] != null)){
			$fname2 = $newfilename2; 
		}else{
			$fname2 = $this->input->post('dsc_signature_part');
		}
	...
	*/	
	...
	public function change_password() {
		...
		} else if(strlen($this->input->post('new_password')) < 7) {
			$Return['error'] = $this->lang->line('xin_employee_error_password_least');
		}else if (!preg_match('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%]).{7,15}$/', $this->input->post('new_password'))){
			$Return['error'] = "Please enter password with 7-15 character (One Capital Character, One Small Character, One Number and One Special Character).";
	...
	public function documents() {
		...
		$doe = strtotime($r->date_of_expiry);
	 ...
	 public function documents_archive() {
		//set data
		$data['title'] = $this->Xin_model->site_title();
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
