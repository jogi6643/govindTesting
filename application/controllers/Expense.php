...
class Expense extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = $this->lang->line('xin_manual');
    ...
    public function expense_list()
     {
		...
		$user = $this->Xin_model->read_user_info($session['user_id']);
		 ...
		 if($session['user_role_id'] == 1){
			...
			$expense = $this->Expense_model->get_expenses_user($user[0]->department_id);
		 }
		...
		$data = array();
		 ...
		 if($session['user_role_id'] == 1){
             ...
             //      $status,
               ...
               $data[] = array(
			   		//$download.
'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-expense_id="'. $r->expense_id . '"><i class="fa fa-eye"></i></button></span>',
                    $department_name,
              //      $expense_type[0]->name,
                    $r->amount,
                    $edate,
	...
	public function add_expense() {
		...
		/* Check if file uploaded..*/
			...
			if(is_uploaded_file($_FILES['bill_copy']['tmp_name'])) {
				...
				if(in_array($ext,$allowed)){
					...
					$newfilename = 'manual_'.round(microtime(true)).'.'.$ext;
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_success_add_manual');
		} else {
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_success_update_manual');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('xin_success_delete_manual');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
