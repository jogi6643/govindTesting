...
class Project extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = 'Document Diary';
    ...
    public function project_list()
     {
		...
		$project = $this->Project_model->get_projects_by_user_id($session['user_id']);
		}
	  ...
	  foreach($project->result() as $r) {
		...
		$data[] = array(
			...
			'<span data-toggle="tooltip" data-placement="top" title="Edit"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-project_id="'. $r->project_id . '"><i class="fa fa-pencil-square-o"></i></button></span>',
			$project_summary,
			$priority,
			$pdate,
			$pbar,
	 ...
	 public function read()
	{
		...
		$data = array(
				...
				'file_letter_reference_no' => $result[0]->file_letter_reference_no,
				'mobile_no' => $result[0]->mobile_no,
				'fax_no' => $result[0]->fax_no,
				'contact_no' => $result[0]->contact_no,
				'organization_address' => $result[0]->organization_address,	
	...
	public function add_project() {
		...
		$bill_copy = "./uploads/document_diary/";
		//$lname 	= basename($_FILES["document"]["name"]);
		//$newfilename = $lname;
		$newfilename_receipt = "document_diary"."_".round(microtime(true)).'.'.$ext_receipt;
		move_uploaded_file($tmp_name_receipt, $bill_copy.$newfilename_receipt);
		}

		if(($_FILES['document']['size'] != '0') || ($_FILES['document']['size'] != null) ){
			$fname = $newfilename_receipt;
		}else{
			$fname = '';
		}
		...
		'file_letter_reference_no' => $this->input->post('file_letter_reference_no'),
		...
		if ($result == TRUE) {
			...
			$Return['result'] = "Document Diary Added";
			$this->output($Return);
			exit;
			//get setting info 
	...
	public function update() {
		...
		// }else{
			// $fname = '';
		// }
		if(($this->input->post('document') != '0') || ($this->input->post('document') != null) ){
			$fname = $this->input->post('document');
		}else{
			$fname = '';
		}
		...
		if ($result == TRUE) {
			...
			$Return['result'] = "Document Diary Updated";
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if(isset($id)) {
			...
			$Return['result'] = "Document Diary Deleted";
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
