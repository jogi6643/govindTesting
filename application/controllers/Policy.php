    ...
    public function policy_list()
     {
		...
		$policy = $this->Policy_model->get_policies_user($user[0]->department_id);
		}
		...
		$data[] = array(
			'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-policy_id="'. $r->policy_id . '"><i class="fa fa-eye"></i></button></span>',
			$r->title,
			$company,
			$department_name,
			$pdate,
