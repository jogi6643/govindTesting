...
class Sanction_files_by_department extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = 'Report by Department';
		$data['path_url'] = 'sanction_files_by_department';
		$session = $this->session->userdata('username');
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('106',$role_resources_ids)) {
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("e-file/sanction_files_by_department", $data, TRUE);
	 ...
	 public function file_list_pdf() {
		...
		// $file = $this->Files_model->get_files_list_download();
		 ...
		 $file = $this->Sanction_files_model->get_sanction_files_reporting_officer_by_department($session['user_id']);
	...
	foreach($file->result() as $r){
		...
		$department_name = $this->Sanction_files_model->get_department_name($department_id);
		...
		$tbl .='<tr>
				...
				<td>'.$r->status.'</td>
				<td>'.$department_name[0]->department_name.'</td>
		...
		$pdf->writeHTML($tbl, true, false, false, false, '');
		//Close and output PDF document
		$pdf->Output('e-file'.'_'.'department_file_list'.'_'.date('Y-m-d').'.pdf', 'D');		  
     }
    ...
    public function sanction_file_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("e-file/sanction_files_by_department", $data);
		} else {
			redirect('');
		}
		...
		// $depart_id = $this->input->post('department_id');
		$depart_id = $this->input->get('department_id');
		// $file_method = $this->input->get('file_method');
		...
		if($depart_id != null){
		 ...
		 if($session['user_role_id'] == 1){
			...
			foreach($get_employee_id as $row){
				...
				$user_depart_id = $user[0]->department_id;
				//print_r($employee_id.' = '.$user_depart_id.'<br>');

				if($user_depart_id == $depart_id){
...
//						'<span data-toggle="tooltip" data-placement="top" title="Edit"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-id="'. $r->id  . '"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="View"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-id="'. $r->id . '"><i class="fa fa-eye"></i></button></span><a href="javascript:void(0);" onclick="deletep('.$r->id.');" title="Delete"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal"><i class="fa fa-trash-o"></i></button></a>',
		 ...
		 $get_employee_result = $this->Sanction_files_model->get_sanction_files_reporting_officer_by_department_group_by($session['user_id']);
		 $get_employee_id = $get_employee_result->result();
		 ...
		 foreach($get_employee_id as $row){
			...
			//print_r($employee_id.' = '.$user_depart_id.'<br>');
				...
				$file = $this->Sanction_files_model->get_sanction_files_reporting_officer_by_department_file_in($employee_id,$session['user_id']);
