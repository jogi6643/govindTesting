    ...
    public function department_list()
     {
		...
		$data = array();
          ...
          foreach($department->result() as $r) {
			  ...
			  // get location
			  $location = array();
			  $location = $this->Xin_model->read_location_info_array($r->location_id); 
			  // get user > head
			  $head_user = $this->Xin_model->read_user_info($r->employee_id);
			  // user full name 
			  foreach($head_user as $headuser => $hu){ 
			  $dep_head = $hu->first_name.' '.$hu->last_name;	
			  }		
			  // get user > added by
