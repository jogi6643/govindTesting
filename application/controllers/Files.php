...
class Files extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = 'Files';
		$data['path_url'] = 'files';
		$session = $this->session->userdata('username');
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('102',$role_resources_ids)) {
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("e-file/files", $data, TRUE);
	 ...
	 public function pdf_create() {
		...
		foreach($signature->result() as $signaturee){
		 ...
		 $tbl .= '<br><img src="'.base_url().'uploads/signature/'.$signature_name.'" width="100" height="50">'.'<p style= "float:left;">'.$updated_time.'<br>'."e-Sign by ".$employee_names.'<br>'.$designation_id[0]->designation_name.'</p>';
		}
    ...
    public function file_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("e-file/files", $data);
		} else {
			redirect('');
		}
	...
	$data = $this->Files_model->get_all_user($user_id);
	}	
	if($selection_dropdown == '3'){
	$data = $this->Files_model->get_all_user($user_id);
	}
	if(count($data) == 0){
		$pro_select_box = "";
	...
	private function filename_exists($filename){
	$this->db->where('file_name', $filename);
	$query = $this->db->get('xin_files');
	...
	public function add_file() {
		...
		$file_name = $post_data['file_name'].$post_data['auto_number'];
		if($this->input->post('file_name')==='') {
        	$Return['error'] = "The file no field is required.";
		}else if($this->filename_exists($file_name) == TRUE ) {
			$Return['error'] = "File No already exist.";
	        }
		...
		}else if(($this->input->post('status')!='Draft') && ($this->input->post('selection_dropdown')==='')) {
			$Return['error'] = "The reporting officer field is required.";	
		}else if(($_FILES['attachment_file1']['size'] > 20000000)) {
		...
		$selection_dropdown = $this->input->post('selection_dropdown');
		$report = $this->input->post('reporting_officer');

		foreach($report as $reports => $reported){ //print_r($report[0]);die;
			$reporting_officer_1_level = $report[0];
		}
        ...
        'reporting_officer' => $reporting_officer_1_level,
		...
		if($e_sign != null){
			...
			$message = "Welcome to Dredging Coporation of India. Your Verification Code :".$random_string;
					
			$this->email->from('eoffice@dcil.co.in'); 
		...
		if($selection_dropdown == '3'){
			...
			foreach($report as $reports => $reported){ //print_r($report[0]);die;
				...
				'employee_id' => $post_data['employee_id'],
				'current_employee_id' => $session['user_id'],
				'reporting_officer' => $reported,
				'created_at' => date("Y-m-d H:i:s")
				);
				$result_reporting_officer = $this->Files_model->add_reporting_officer($data3); $j++;
				}else{
				$data3 = array(
				'file_id' =>  $get_files_id[0]->id,
				'employee_id' => $post_data['employee_id'],
				'current_employee_id' => $report[$i],
				'reporting_officer' => $reported,
				'created_at' => date("Y-m-d H:i:s")
				);
				$result_reporting_officer = $this->Files_model->add_reporting_officer($data3); $i++;					
				}	
			}
		}
