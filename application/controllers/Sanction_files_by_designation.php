...
class Sanction_files_by_designation extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = 'Report by Designation';
		$data['path_url'] = 'sanction_files_by_designation';
		$session = $this->session->userdata('username');
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('107',$role_resources_ids)) {
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("e-file/sanction_files_by_designation", $data, TRUE);
	 ...
	 // create pdf - file
		...
		$pdf->SetCreator('e-File Management System'); 
		print_r($system[0]->payroll_logo);die;
	 ...
	 public function file_list_pdf() {
		...
		// $file = $this->Files_model->get_files_list_download();
		 ...
		 $file = $this->Sanction_files_model->get_sanction_files_reporting_officer_by_designation($session['user_id']);		
	...
	foreach($file->result() as $r){
		...
		$designation_name = $this->Sanction_files_model->get_designation_name($designation_id);
		...
		$tbl .='<tr>
				...
				<td>'.$r->status.'</td>
				<td>'.$designation_name[0]->designation_name.'</td>
		...
		$pdf->writeHTML($tbl, true, false, false, false, '');
		//Close and output PDF document
		$pdf->Output('e-file'.'_'.'designation_file_list'.'_'.date('Y-m-d').'.pdf', 'D');		  
     }
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("e-file/sanction_files_by_designation", $data);
		} else {
			redirect('');
		}
		...
		$design_id = $this->input->get('designation_id');
		
		if($design_id != null){
			
		 if($session['user_role_id'] == 1){
			
			$get_employee_result = $this->Sanction_files_model->get_sanction_files();	
			$get_employee_id = $get_employee_result->result();
			$data = array();
			foreach($get_employee_id as $row){
				$employee_id = $row->employee_id;
				$user = $this->Xin_model->read_user_info($employee_id);
				$user_design_id = $user[0]->designation_id;
				// print_r($employee_id."=".$design_id.'<br>');	

				if($user_design_id == $design_id){		
				$file = $this->Sanction_files_model->get_sanction_files_reporting_officer_by_department_file_in_by_admin($employee_id);
				foreach($file->result() as $r) {
				...
				$designation_id = $user[0]->designation_id;
				$designation_name = $this->Sanction_files_model->get_designation_name($designation_id);
				$department_name = $this->Sanction_files_model->get_department_name($designation_name[0]->department_id);
				if($reporting_officer != null){
				$reporting_officer_full_name = htmlentities($reporting_officer[0]->first_name). ' '.htmlentities($reporting_officer[0]->last_name);
				}else{
				$reporting_officer_full_name = "";	
...
//					'<span data-toggle="tooltip" data-placement="top" title="Edit"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-id="'. $r->id  . '"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="View"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-id="'. $r->id . '"><i class="fa fa-eye"></i></button></span><a href="javascript:void(0);" onclick="deletep('.$r->id.');" title="Delete"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal"><i class="fa fa-trash-o"></i></button></a>',
					...
					$r->status,
					$r->priority,
					$designation_name[0]->designation_name." (".$department_name[0]->department_name.")",
					$reporting_officer_full_name,
					$updated_date,
					//$payslip,
			...
			exit();
				...
				$get_employee_result = $this->Sanction_files_model->get_sanction_files_reporting_officer_by_designation_group_by($session['user_id']);	
				$get_employee_id = $get_employee_result->result();
			...
			if($user_design_id == $design_id){		
				$file = $this->Sanction_files_model->get_sanction_files_reporting_officer_by_designation_file_in($employee_id,$session['user_id']);
