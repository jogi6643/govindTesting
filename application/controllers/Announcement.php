...
class Announcement extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = $this->lang->line('xin_circulars');
    ...
    public function announcement_list()
     {
		...
		$announcement = $this->Announcement_model->get_announcements_user($user[0]->department_id);
		}
		$data = array();
		...
		$data[] = array(
			'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-announcement_id="'. $r->announcement_id . '"><i class="fa fa-eye"></i></button></span>',
			$r->title,
			$r->summary,
			$department_name,
			$sdate,
