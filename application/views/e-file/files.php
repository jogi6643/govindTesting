               ...
               <div class="row">
				<div class="col-md-2">
					...
					<label for="file_name">File No.</label>
						...
						$file_no = substr($department_name, 0, 3)."/".date('Y')."/";
						?>	
					...
					<select id="file_type" class="form-control" name="file_type"  data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('xin_file_type');?>">
						...
						<?php if($user[0]->department_id == $row->department_id):?>	
...
$(function() {
    $( "#datepicker" ).datepicker({ minDate: 0});
				...
				<div class="row">
					...
					<div class="col-md-4">
                      <div class="form-group">
						<label for="remark">Reference file</label><br><small>(max upload file size is 20 MB and PDF,Doc & JPEG only)</small><input accept="image/*, application/pdf" name="attachment_file1" class="uploadjs form-control" data-id="3" type="file">
...
$(document).ready(function(){
    ...
    var addButton = $('.add_select'); //Add button selector
    var wrapper = $('.field_select'); //Input field wrapper
    var fieldHTML = '<div><select name="reporting_officer[]" id="reporting_officer" class="form-control" data-plugin="select_hrm" data-placeholder="Choose Reporting Officer"><option value="">Choose Reporting Officer</option><?php foreach($all_employees as $employee) {?><option value="<?php echo $employee->user_id;?>"> <?php echo $employee->first_name.' '.$employee->last_name;?></option><?php } ?></select><a href="javascript:void(0);" class="remove_button"><img src="<?php echo base_url().'uploads/image/remove-icon.png';?>"/> Remove</a></div><br>'; //New input field html 
    var x = 1; //Initial field counter is 1
...
<div class="row">
				...
				<div class="field_wrapper">
					<div>
						...
						<small>1.(max upload file size is 20 MB and PDF only)<br>2.(Don't use white space in file name for example for file name my resume use my_resume)</small>
	...
	<!--			<div class="col-md-3">
                    <div class="form-group">
					<label for="preferred_list">Preferred List</label> 
					<textarea class="form-control" placeholder="Preferred List"></textarea> 	
					</div>
					</div>
	-->
	<!--			<div class="col-md-3">
                <div class="form-group">
                  <label for="employee"><?php echo "Preferred List";?></label>
                  <select multiple name="assigned_to[]" id="select2-demo-6" class="form-control select-border-color border-warning" data-plugin="select_hrm" data-placeholder="<?php echo "Preferred List";?>">
                    <option value=""></option>
	...
	-->			<div class="col-md-3">
					...
					<select name="selection_dropdown" id="selection_dropdown" class="form-control" data-plugin="select_hrm" data-placeholder="Choose Reporting Officer">
                      ...
                      <option value="3">Set Hierarchy</option>
                    </select>		
					</div>
					</div>
                   ...
                   <div class="form-group" style="padding: 26px 0px 0px 0px;">
					<label id="level" hidden="">Add Level <a href="javascript:void(0);" class="add_select" title="Add field"><img src="<?php echo base_url().'uploads/image/add-icon.png';?>"/></a></label>	
...
-->				<input type="hidden" name="verify" id="myBtn" value="">
			...
			<div id="myDIV" style="display:none;"> 
			 <div class="row">						
              ...
              <button type="submit" class="btn btn-primary">e-Sign & Submit</button>		
...
<div class="box box-block bg-white">
  <h2><strong>List All</strong> Files 
  <div class="add-record-btn">
