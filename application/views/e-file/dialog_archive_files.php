...
<div class="modal-header">
  ...
  <h4 class="modal-title" id="edit-modal-data">Edit Archive File</h4>
</div>
...
<div class="modal-header">
  <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
  <h4 class="modal-title" id="edit-modal-data">View Archive File</h4>
</div>
<form class="m-b-1">
  <div class="modal-body">
        <div class="row">
...
<?php $file_log = $this->Files_model->get_files_by_id($id);
	...
	foreach($file_log as $file_logs){
	$user = $this->Xin_model->read_user_info($file_logs->current_employee_id);
	$payslip = '<a class="text-success" id="e-file" onclick = "openPdf()" value="'.site_url().'sanction_files/pdf_create_id/id/'.$file_logs->id.'/'.$file_logs->file_id.'" href="javascript:void(0)">Notesheet by '.$user[0]->first_name." ".$user[0]->last_name.'</a><br>';
	echo $payslip;
	$i++;
	}
?>
...
var omyFrame = document.getElementById("e-pdf-iframe");
omyFrame.style.display="block";
omyFrame.src = $('#e-file').attr('value');
