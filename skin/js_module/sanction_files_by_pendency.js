...
$(document).ready(function() {
    var a = $("#xin_table")['dataTable']({
        "bDestroy": true,
        "ajax": {
            url: site_url + "sanction_files_by_pendency/sanction_file_list/",
            type: "GET"
        },
        "fnDrawCallback": function(b) {
			...
			$('#pendency_range').on('change',function(){
						var pendency_range = $(this).val();
						//var file_method = $('#file_method').val();
						if(pendency_range == '')
						{
						pendency_range == '';
						var oTable = $('#xin_table').dataTable();
						oTable.api().ajax.url(site_url + "sanction_files_by_pendency/sanction_file_list?pendency_range="+ pendency_range).load();
						//console.log(oTable);
						}else{
							var oTable = $('#xin_table').dataTable();
							oTable.api().ajax.url(site_url + "sanction_files_by_pendency/sanction_file_list?pendency_range="+ pendency_range).load();
