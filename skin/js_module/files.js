...
$(document).ready(function() {
    ...
    $(".edit-modal-data")["on"]("show.bs.modal", function(d) {
        ...
        $["ajax"]({
            url: base_url + "/read/",
            type: "GET",
            data: "jd=1&is_ajax=1&mode=modal&data=edit_file&id=" + f,
            success: function(g) {
                if (g) {
                    $("#ajax_modal")["html"](g)
                }
	...
	$('#selection_dropdown').on('change',function(){
		...
		if(selection_dropdown == '')
		{
			$('#reporting_officer').prop('hidden', true);
				...
				$.ajax({
					url: site_url + "files/get_reporting_officer",
					type:"POST",
				...
				}else if(selection_dropdown != 3 || selection_dropdown != ''){	
				$('#reporting_officer').prop('hidden', false);
