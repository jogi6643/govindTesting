...
$(document).ready(function() {
    var a = $("#xin_table")['dataTable']({
        "bDestroy": true,
        "ajax": {
            url: site_url + "draft_files/file_list/",
            type: "GET"
        },
    ...
    $(".edit-modal-data")["on"]("show.bs.modal", function(d) {
        ...
        $["ajax"]({
            url: base_url + "/read/",
            type: "GET",
            data: "jd=1&is_ajax=1&mode=modal&data=edit_draft_file&id=" + f,
            success: function(g) {
                if (g) {
                    $("#ajax_modal")["html"](g)
                }
    ...
    $(".view-modal-data")["on"]("show.bs.modal", function(d) {
        ...
        var e = $(this);
        $["ajax"]({
            url: site_url + "draft_files/read/",
            type: "GET",
            data: "jd=1&is_ajax=1&mode=view_modal&data=view_file&id=" + f,
            success: function(g) {
                if (g) {
	...
	$("#remark")["summernote"]({
		...
		var file_type = $(this).val();
			if(file_type){	
				...
				$.ajax({
					url: site_url + "draft_files/get_template",
					type:"POST",
	...
	$('#selection_dropdown').on('change',function(){
		var selection_dropdown = $(this).val();
		if(selection_dropdown == '')
		{
			$('#reporting_officer').prop('hidden', true);
		}
			else{	
				$('#reporting_officer').prop('hidden', false);
				$.ajax({
					url: site_url + "draft_files/get_reporting_officer",
					type:"POST",
					data:{'selection_dropdown' : selection_dropdown},
					datatype:'json',
					success:function(data){
						$('#reporting_officer').html(data);
					},
					error:function(){
						alert('Error Occur');	
					}
