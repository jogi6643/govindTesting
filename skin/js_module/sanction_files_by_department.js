...
$(document).ready(function() {
    var a = $("#xin_table")['dataTable']({
        "bDestroy": true,
        "ajax": {
            url: site_url + "sanction_files_by_department/sanction_file_list/",
            type: "GET"
        },
		...
		$('#department_id').on('change',function(){
						...
						//if(file_method != null){ alert(file_method);}
						if(department_id == '')
						{
						department_id == '';
						var oTable = $('#xin_table').dataTable();
						oTable.api().ajax.url(site_url + "sanction_files_by_department/sanction_file_list?department_id="+ department_id).load();
						//console.log(oTable);
						}else{
							var oTable = $('#xin_table').dataTable();
							oTable.api().ajax.url(site_url + "sanction_files_by_department/sanction_file_list?department_id="+ department_id).load();
							//	http://192.168.1.4/hrm/sanction_files_by_department/sanction_file_list?department_id=4&ftype=in
							// oTable.api().ajax.url(site_url + "sanction_files_by_department/sanction_file_list?department_id=department_id&file_method=" + file_method).load();							
						}
		});
